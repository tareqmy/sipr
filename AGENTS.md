...
**Current state: v1 shipped, M0–M24 complete** (UDP/TCP/TLS transports, UAC/UAS,
stats + TUI, actions/auth incl. IMS AKA + AUTS resync, `-inf` injection, 3PCC, `-users`, IPv6,
pcap replay, RTP streaming + DTMF, RTP echo + rtpcheck, SRTP, SIPp control socket +
HTTP API). `PLAN.md` is the
