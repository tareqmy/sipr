...
/// writes routed back by peer address.
        ...
        let accept = std::thread::Builder::new()
            ...
            .spawn(move || {
                for stream in listener.incoming() {
                    ...
                    let _ = register(&accept_conns, peer, stream, &sink);
                }
            })
            .ok();
        Ok(Self {
...
fn register(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || read_loop(read_half, peer, &sink, &conns))
        .ok();
    Ok(())
}

/// Frame everything arriving on one connection until it closes or errors. A
/// per-connection failure deregisters that peer but never signals the engine —
/// one client hanging up must not stop a server (timeouts handle stalls).
fn read_loop(mut stream: TcpStream, peer: SocketAddr, sink: &Sender<NetEvent>, conns: &Conns) {
